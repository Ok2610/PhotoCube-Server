...
namespace ObjectCubeServer.Models.PublicClasses
{
    ...
    public class PublicCell
    {
        ...
        public PublicCell(int x, int y, int z, int count, int id, string fileURI, string thumbnailURI)
        {
            this.x = x;
            this.y = y;
            this.z = z;
            this.count = count;
            CubeObjects = new List<PublicCubeObject> {new(id, fileURI, thumbnailURI)};
        }
    }
}
