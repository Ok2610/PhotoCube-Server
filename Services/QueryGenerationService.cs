...
namespace ObjectCubeServer.Services
{
    public class QueryGenerationService
    {
        ...
        internal string generateSQLQueryForState(string xType, int xVertexId, string yType, int yVertexId, string zType, int zVertexId, IList<ParsedFilter>? filtersList)
        {
            ...
            queryEnd.Append(") X join medias O on X.object_id = O.id;");
        ...
        private string generateAxisQueryForCell(string type, int vertexId)
        {
            ...
            switch (type)
            {
                ...
                case "tag":
                    query +=
                        $" select R.object_id from taggings R where R.tag_id = {vertexId}) R{numberOfFilters} ";
                    break;
            }
            return query;
        }
        ...
        private string generateFilterQueryForState(ParsedFilter filter)
        {
            ...
            switch (filter.type)
            {
                ...
                case "tag":
                    ...
                    {
                        query +=
                            $" select R.object_id from taggings R where R.tag_id = {filter.Ids[0]}) R{numberOfFilters}";
                    }
                    else
                    {
                        query +=
                            $" select R.object_id from taggings R where R.tag_id in {generateIdList(filter)}) R{numberOfFilters}";
                    }
                    break;

                case "numrange":
                    query +=
                        $" select R.object_id from numerical_tags T join taggings R on T.id = R.tag_id where {generateRangeList(filter, "")}) R{numberOfFilters}";
                    break;

                case "alpharange":
                    query +=
                        $" select R.object_id from alphanumerical_tags T join taggings R on T.id = R.tag_id where {generateRangeList(filter, "'")}) R{numberOfFilters}";
                    break;

                case "daterange":
                    query +=
                        $" select R.object_id from date_tags T join taggings R on T.id = R.tag_id where {generateRangeList(filter, "'")}) R{numberOfFilters}";
                    break;

                case "timerange":
                    query +=
                        $" select R.object_id from time_tags T join taggings R on T.id = R.tag_id where {generateRangeList(filter, "'")}) R{numberOfFilters}";
                    break;
                case "timestamprange":
                    query +=
                        $" select R.object_id from timestamp_tags T join taggings R on T.id = R.tag_id where {generateRangeList(filter, "'")}) R{numberOfFilters}";
                    break;
            }
